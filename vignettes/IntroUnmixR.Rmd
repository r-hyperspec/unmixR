...
In practice, abundance estimation supports both pure-pixel and non-pure-pixel workflows:

* Pure-pixel methods (`nfindr`, `vca`, `atgp`): pass the model output directly, e.g. `abundances(nfindr_fit, x)`.
* Non-pure-pixel methods (`ice`): pass the endmember matrix, e.g. `abundances(ice_fit$endmembers, x)`.
