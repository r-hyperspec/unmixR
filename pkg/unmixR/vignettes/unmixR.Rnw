...
\Keywords{hyperspectral unmixing, vertex component analysis, VCA, NFINDR, N-FINDR, iterative constrained endmembers, ICE, spectroscopy, \proglang{R}, spectral mixture analysis, Raman, IR, NIR, NMR, MS}

\Plainkeywords{hyperspectral unmixing, vertex component analysis, VCA, NFINDR, N-FINDR, iterative constrained endmembers, ICE, spectroscopy, R, spectral mixture analysis, Raman, IR, NIR, NMR, MS} %% without formatting
...
The iterated constrained endmember (ICE) algorithm differs from N-FINDR and VCA in several significant ways (\cite{Berman2004, Berman2009}).  It begins by reducing the data using the minimum noise fraction (MNF, \cite{Green1988}).  This is a variant of PCA which sorts the abstract components according to the SNR ratio (PCA sorts components by the amount of variance explained).  Computing the MNF is only possible when the number of observations ($n$) is greater than the number of channels ($p$).  When this is not true, the \pkg{unmixR} implementation of ICE reverts to using PCA.  Luo \emph{et al.} discuss the applications of MNF versus PCA in hyperspectral unmixing and note that the performance depends upon the type of noise in the data set (\cite{Luo2016}).
...
The relative performance of the algorithms is presented in Figures~\ref{laserBenchmark} and \ldots  These benchmarks were measured on a MacBook Pro (early 2015) with a 2.9 GHz Intel Core i5 processor and 16GB RAM. \emph{Should we consider the version of BLAS involved?}
