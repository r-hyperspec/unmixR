...
where $red$ reminds us the dimensions have been reduced and we are now in an abstract space of $p'$ dimensions.  However, for consistency with the literature we will continue to use the original symbols.
...
Vertex component analysis (VCA) is an alternative method of locating the endmembers, first described by Nascimento and Dias, with an improved algorithm reported by Lopez and colleagues (\cite{Nascimento2005, Lopez2012}).

In the VCA series of algorithms, the signal-to-noise ratio of the data (SNR), which depends on $p$, determines how the data is pre-processed.  However, \emph{the data is always projected} and \emph{always reduced} as explained in a moment.  In what follows, note the distinction between reduction and projection.
...
The iterated constrained endmember (ICE) algorithm takes a rather different approach to extracting the endmembers, namely to minimize the error between the observed data, $X_{obs}$, and the predicted data, $X_{pred}$ (\cite{Berman2004, Berman2009})  This is accomplished by...
