...
where $i$ is one of the $m$ endmembers of the system.  This way of describing the system is referred to as the "linear mixing model" or the "convex geometry model."
...
A number of unmixing algorithms have been described in the literature.  The chief difference between them is the method of finding the relevant simplex, which in turn identifies the endmembers.  In \pkg{unmixR}, there are variations of the N-FINDR, vertex component analysis (VCA) and iterated constrained endmembers (ICE) algorithms.  Once the endmembers are identified, the actual unmixing can be done to generate an abundance map.  These steps are discussed in the next sections.

\subsection[N-FINDR]{N-FINDR}

The N-FINDR method finds the smallest simplex that encloses the dimensionally reduced data by an inflation process (\cite{Winter1999}).  In this process $p$ samples are chosen at random, and the volume of the simplex is computed (via the absolute value of the determinant).  Then a new sample is selected to replace one of the already chosen ones, and the volume is re-computed.   If the volume increases, the new sample is retained, otherwise the original sample is retained.  The process is repeated until the simplex is inflated to a maximum volume, which will be the smallest simplex that encloses the data set.  At this point, the vertices of the simplex are the purest pixels in the scene.

\textbf{Could include a simple figure here showing steps in growing the simplex.}

\pkg{unmixR} includes several variants of this approach.  For testing purposes, the "Brute" method computes all possible simplices.  This will naturally be quite slow for all but trivial data sets.  Method "99" implements Winter's original algorithm (\cite{Winter1999} which iterates several times from different starting points.  This approach is necessary in the event that there are no pure pixels in the data set.

Discuss other variations in this algorithm (LDU, seqLDU).
...
Demonstrate bench-marking here.
