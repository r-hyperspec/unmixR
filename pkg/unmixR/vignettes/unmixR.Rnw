...
The N-FINDR method finds the smallest $p$-dimensional simplex that encloses the reduced data using an iterative inflation process (\cite{Winter1999}).  The method assumes that pure pixels are present in the scene, an assumption that doesn't always hold.  In this process $p$ pixels are chosen at random, and the volume of the simplex is computed (via the absolute value of the determinant).  Then a new pixek is selected to replace one of the already chosen ones, and the volume is re-computed.   If the volume increases, the new pixel is retained, otherwise the original pixel is retained.  These steps are repeated using an interative approach until the simplex is inflated to a maximum volume, which will be the smallest simplex that encloses the data set.  At this point, the vertices of the simplex are the purest pixels in the scene.  
...
Demonstrate bench-marking here.  Note: temporarily, there is a separate vignette to do the benchmarking.
