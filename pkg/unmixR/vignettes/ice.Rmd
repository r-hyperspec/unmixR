...
Te abundances are calculated via quadratic programing. There are two possible situations. First one is that the assumption is correct. In this case vertices inside the simplex are convex combinations of simplex vertices.
...
True barycentric coordinates can be found solving the system of linear equasions.
```{r}
...
nnls::nnls(m1, p1)$x
```
Now we have to add both constraints to the least squares method. Note that the matrix m is used not m1.
Lsei stands for least squares with equality and inequality constraints. It solves $\left \| Ax - b \right \|^{2}$ problem with $Ex=f$ and $Gx\geq h$ constraints.

The abundances sum to one means$$\begin{pmatrix}
1 & 1 & ... & 1
...
In this case the method solve is not able to find correct answer but two other methods are. Let's consider another point outside a simplex.
...
$$Y = MA$$
$$A^{T}M^{T} = Y^{T}$$
However the matrix A is not square one end not invertible. To calculate M we will use left inverse matrix $$(A^{T})_{left}^{-1} = (AA^{T})^{-1} A$$
$$AA^{T}M^{T} = AY^{T}$$
$$(AA^{T})^{-1}AA^{T}M^{T} = (AA^{T})^{-1}AY^{T}$$
$$M^{T} = (AA^{T})^{-1}AY^{T}$$
To make the model more resistant to nise in data a regularization paremeter is used. Futher information about regularization peremeter can be fount in [1].
...
```{r}
    ...
    qplot(data =  as.data.frame(reducedData), x = x, y = y, geom = "point", main = "laser data reduced to dimensionality of 2")
```
```{r}
    set.seed(1234)
    res <- ice(laser, 3)
    ...
    ggplot() + geom_point(data =  as.data.frame(reducedData), mapping = aes(x = x, y = y)) + geom_polygon(data = as.data.frame(reducedEndmembers), mapping = aes(x = x, y = y), alpha = 0.05, color = "red") + ggtitle("Ice simplex")
```
...
This parameter penaltizes the model for the simplex size. The regularizatoin technique makes the model less affected by noise in data. The higher the value the bigger the penalty so when it is equal to 1 the simplex shrinks to a single point.
...
The dependence of the simplex size on the regulqrization parameter can be seen on the following picture. Since the value is usually very small we consider values equal to powers of 10.

```{r}
    ggplot() + geom_point(data = as.data.frame(reducedData), mapping = aes(x = x, y = y)) + geom_polygon(data = head(simplices, -3), mapping = aes(x = x, y = y, colour = muValue, group = muValue), alpha = 0.05) + scale_color_gradientn(colours = rainbow(length(m)), trans = "log10") + ggtitle("Dependence on mu")
```
...
```{r}
    ggplot() + geom_point(data = as.data.frame(reducedData), mapping = aes(x = x, y = y)) + geom_polygon(data = simplices, mapping = aes(x = x, y = y, colour = muValue, group = muValue), alpha = 0.05) + scale_color_gradientn(colours = rainbow(length(m)), trans = "log10") + ggtitle("Dependence on mu")
```

###Tolerance ratio

The tolerance ratio determines the number of iterations. The algorithm asymptotically approaches the solution and higher values of the parameter mean that the result will be closer to the optimal one and wikk require more iterations.
...
The value of tau is usually extremely close to 1 and the following plot shows it very well. Four simplices correslonding to lower values differ very much. Simplices which correspond to 0.9999 and 0.99999 differ very little. That means that the algorithm finds the optimal solution having the value of tau not less than 0.999.

```{r}
    ggplot() + geom_point(data = as.data.frame(reducedData), mapping = aes(x = x, y = y)) + geom_polygon(data = simplices, mapping = aes(x = x, y = y, colour = tauValue, group = tauValue), alpha = 0.05) + scale_color_gradientn(colours = rainbow(3), values = t) + ggtitle("Dependence on tau")
```

As the number of iterations grows so does the calculation time.

```{r}
    rbenchmark::benchmark(ice(laser, 3, t = 0.9), ice(laser, 3, t = 0.99), ice(laser, 3, t = 0.999), ice(laser, 3, t = 0.9999), ice(laser, 3, t = 0.99999))
```