...
```{r, cache=TRUE}
    set.seed(1234)
    res <- ice(laser, 3, mu = 0.00001, t = 0.999)
    endm <- res$endmembers
...
```{r, cache=TRUE}
    ggplot() + geom_point(data = as.data.frame(reducedData), mapping = aes(x = x, y = y)) + geom_polygon(data = head(simplices, -3), mapping = aes(x = x, y = y, colour = muValue, group = muValue), alpha = 0.05) + scale_color_gradientn(colours = rainbow(length(m)), trans = "log10") + ggtitle("ICE simplices")
```
...
```{r, cache=TRUE}
    ggplot() + geom_point(data = as.data.frame(reducedData), mapping = aes(x = x, y = y)) + geom_polygon(data = simplices, mapping = aes(x = x, y = y, colour = muValue, group = muValue), alpha = 0.05) + scale_color_gradientn(colours = rainbow(length(m)), trans = "log10") + ggtitle("ICE simplices")
```
