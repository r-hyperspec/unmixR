...
dM1 <- genTestData(50, 3, 4, TRUE)
str(dM1)
```

#### Data Reduction

In the VCA series of algorithms, the signal-to-noise ratio of the data (SNR) determines how the data is pre-processed, but *it is always projected* as explained in a moment.  The SNR is a function of $p$.  In what follows, note the distinction between transformation, reduction and projection.

If the SNR is high, the data is transformed using an SVD process without changing the value of $p$ (this amounts to transforming the data into an abstract space, similar to PCA, but keeping all the components).  It is then projected on to lower dimension.  For example, if the raw data is composed of random 3D data, the data will be projected onto a plane which resides in 3D space.  Thus, in a sense the data has become two-dimensional since every point lies in a plane.  One might call this "flattening".  Nascimento and Dias, in describing the VCA algorithm, use the awkward phrase "projective projection".  The process just described is the one of the projection steps.  The other is coming up shortly.

If the SNR is low, the data is first reduced to dimension $p - 1$ using SVD thereby improving the SNR.  The data is projected as before, then a constant vector is added back so that the original dimension $p$ is restored.
...
```{r dimRed}
dM2 <- dimensionalityReduction(dM1$data, 3)
```

We've done the reduction step manually here, but it is carried out automatically in _unmixR_.
...
```{r compare_dM_webgl, webgl = TRUE}
plot3d(dM1$data, col = "red", expand = 1.5)
points3d(dM2*100, col = "green")
```

Note how the processed data has been projected onto a plane (flattened).

```{r temporarilyHere, eval = FALSE, echo = FALSE}
tst <- vca(dM1$data, p = 3)
colnames(tst$data) <- c("X", "Y", "Z")

#plot3d(tst$data, col = "red", expand = 1.2)
points3d(dM2[tst$indices,]*100, col = "blue", size = 10)
# cannot work! points3d(dM1$data[tst$indices,], col = "blue", size = 10)
```
...
3. Iterate from 1 to $p$:
   + Calculate a reference vector orthogonal to the subspace spanned by columns of $\mathbf{X}$, the processed original data.  This vector will pass through (0,0,...,0).
