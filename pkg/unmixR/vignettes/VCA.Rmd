...
options(rgl.useNULL = TRUE) # not sure if this helps
...
At last all endmembers have been identified.

```{r finalResult, echo = FALSE}
M <- res$data
colnames(M) <- c("X", "Y", "Z")
indx <- res$indices
plot3d(M, col = "red", expand = 1.2, aspect = "iso")
points3d(M[indx,], col = "blue", size = 10)
rglwidget()
```

### VCA on the chondro Data Set

The chondro data set from package `hyperSpec` is composed of 875 Raman spectra collected on a grid in a tissue imaging experiment.  There are 300 channels.

```{r chondro}
chondro
```

Since there are 300 channels, we'll ask for three endmembers so we can visualize the results.  As described above, the data is flattened during pre-processing, which will be evident momentarily.  We'll plot the 3D data, and mark the three endmembers identified, but clearly there are really only two dimensions here, and the data is contained within a 2-simplex (triangle) rather than a 3-simplex (tetrahedron).

```{r chondroEMs, echo = FALSE, eval = TRUE}
# Plot the reduced data set
Ch <- chondro@data$spc # extract just the matrix of spectra
ChR <- dimensionalityReduction(Ch, 3)
colnames(ChR) <- c("X", "Y", "Z")
# The magnitude of ChR is quite small, scale up for better viewing
# (i.e. small axis labels); small numbers may also be choking webgl
ChR <- ChR*1e5
# Adjust the X dimension which is basically flat for better viewing
xR <- range(ChR[,"X"])
xDelta <- diff(xR)*100
limX <- xR + c(-xDelta, xDelta)
#plot3d(ChR, col = "red", expand = 1.2, aspect = "iso", xlim = limX)
#plot3d(ChR, col = "red", expand = 1.2, aspect = "iso")

# Mark the endmembers
#Chvca <- vca05mod(data = Ch, p = 3) # error: need to check why
res <- vca05mod(data = ChR, p = 3)
points3d(ChR[res$indices,], col = "blue", size = 10)
