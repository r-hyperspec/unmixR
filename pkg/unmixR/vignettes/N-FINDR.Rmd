...
Since there are 300 channels, in order to visualize the data we'll have to reduce it using PCA, and we'll keep just 3 principal components so we can plot as before (the reduction is automatically carried out by the modified function).  We'll ask for four endmembers so we can draw the simplex.

```{r simplex_chondro, webgl = TRUE, echo = TRUE}
Ch <- chondro@data$spc # extract just the matrix of spectra
results3 <- nfindr99allEM(data = Ch, p = 4, indices = 1:4)
ChR <- results3$data # grab the reduced data for plotting
plot3d(ChR, col = "red", expand = 1.2)
points3d(ChR[results3$EMs[4,],], color = "blue", size = 10)
