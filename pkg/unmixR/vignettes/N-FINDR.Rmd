...
%\VignetteKeywords{hyperspectral unmixing, vertex component analysis, VCA, NFINDR, N-FINDR, iterative constrained endmembers, ICE, spectroscopy, R, spectral mixing analysis}
 -->

Background on the N-FINDR Unmixing Algorithm
========================================================

Bryan A. Hanson, June 2017
...
The N-FINDR algorithm for endmember extraction is credited to Winter.<sup id="Winter1999">[1](#fnWinter1999)</sup>

## N-FINDR on a Synthetic Data Set
...
where $\mathbf{X}$ is a matrix of the raw data (dM1 in the code) and $\mathbf{A}$ (wM) is a matrix giving the abundances of each endmember in each sample.  $\mathbf{E}$ (eM) is a matrix of the endmembers, the pure component spectra (think of $\mathbf{E}$ as a library of reference spectra). $\epsilon$ is a Gaussian noise term.  
...
Because of its low dimensionality, we can visualize this data directly in 3D (click and drag to rotate):

```{r dM1webgl, webgl = TRUE, results = 'hide'}
plot3d(dM1, col = "red", expand = 1.2)
```

### Extract the Endmembers

Next, we'll extract the endmembers using a modified version of `nfinder99`, the original algorithm by M. E. Winter.  This modified version includes optional dimensional reduction (normally handled by a separate function) but most importantly, it records the sequence of endmembers as the algorithm iterates.  This allows us to follow the process directly.  Because this is a low dimension data set, it does not need to be reduced. You can see the modified function in the source for this vignette if desired.

```{r hackedFunc, echo = FALSE}
...
##' B. Hanson June 2017
##'
nfindr99allEM <- function(data, p, indices, iters=3*p) {
  ...
  return(list(data = data, EMs = EMs[1:(iter-1),], VOLs = VOLs))
}
...
```{r extract_dM1}
results1 <- nfindr99allEM(data = dM1, p = 4, indices = 1:4)
data.frame(results1$EMs, volume = na.omit(results1$VOLs),
  row.names = rownames(results1$EMs))
```

The first row of the data frame above is the initial row indices of the data set as specified in the function call.  The second data frame row is the row indices of the identified endmembers, which were found in one iteration.  Note that these endmembers are the pure pixels which we specified above when we created the data set.
...
```{r simplex_dM1, webgl = TRUE, results = 'hide'}
plot3d(results1$data, col = "red", expand = 1.2)
points3d(dM1[results1$EMs[2,],], color = "blue", size = 10)
...
```{r simplex_dM2, webgl = TRUE, echo = FALSE}
results2 <- nfindr99allEM(data = dM2, p = 4, indices = 1:4)
plot3d(results2$data, col = "red", expand = 1.2)
points3d(dM2[results2$EMs[3,],], color = "blue", size = 10)
...
```
In this case it took two iterations to find the best answer:

```{r results2EMs, echo = FALSE}
data.frame(results2$EMs, volume = na.omit(results2$VOLs),
  row.names = rownames(results2$EMs))
```
And it's clear that for realistic noisy data, the endmembers and corresponding simplex *do not* enclose all the data points.  However, any other combination of four data points has a lower volume and was rejected.
