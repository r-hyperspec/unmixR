...
vignette: >
  ...
  %\VignetteKeywords{hyperspectral unmixing, vertex component analysis, VCA, NFINDR, N-FINDR, iterative constrained endmembers, ICE, spectroscopy, R, spectral mixture analysis, SMA, Raman, IR, NIR, NMR, MS, XRF}
output:
  html_document:
    toc: true
...
nfindr99mod <- function(data, p, indices, iters=3*p) {
  ...
  # Reduce data before sending here if desired!
...
Since there are 300 channels, in order to visualize the data we'll have to reduce it using PCA, and we'll keep just three principal components so we can plot as before.  We'll ask for four endmembers so we can draw the simplex.
...
results3 <- nfindr99mod(data = ChR, p = 4, indices = 1:4)
ChR <- results3$data # grab the reduced data for plotting
its <- nrow(results3$EMs) # no. iterations - last row has the answer
plot3d(ChR, col = "red", expand = 1.2) # looks better w/o aspect = "iso"
spheres3d(ChR[results3$EMs[its,],], color = "blue", radius = 0.05)
triangles3d(ChR[results3$EMs[its,1:3],], color = "blue", alpha = 0.15)
triangles3d(ChR[results3$EMs[its,2:4],], color = "blue", alpha = 0.15)
triangles3d(ChR[results3$EMs[its,c(1, 2, 4)],], color = "blue", alpha = 0.15)
triangles3d(ChR[results3$EMs[its,c(1, 3, 4)],], color = "blue", alpha = 0.15)
rglwidget()
```
