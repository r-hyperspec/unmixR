...
3. For $i$ from 1 to $p$:
   + Calculate a reference vector orthogonal to $A_i$.  This vector will pass through (0,0,...,0).
   ...
   + Let $k$ be the index of that data point in the dataset.  Set the $k$-th column of $A$ to $k$-th column of data.  This is one of the endmembers.
4. Return $A$.
...
#        res <- cbind(vector + D * slopes, vector - D * slopes, vector)
    ...
    plotVectors <- function(matrix, vector, testdata){
...
#        print(resMatrix)
        ...
        legend(x = "topright",
          ...
          expression(paste("projection onto ", italic(f)))))
    }
```
...
```{r, echo = FALSE}
    curplot = 1
    plotVectors(collection[[curplot]]$m, collection[[curplot]]$v, d)
```
...
If this process were extended to more than three dimensions, each reference vector $f$ would actually be a hyperplane.  Thus the process would be to choose a point, construct an orthogonal hyperplane, and determine which data point has the most extreme projection onto this hyperplane.  That point is chosen as an endmember and the process is repeated until all the desired endmembers have been identified.
