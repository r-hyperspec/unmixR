...
.test (nfindr.formula) <- function () {
  ...
  set.seed(32)
  formula_nfindr <- suppressWarnings(nfindr(~ x, .testdata, p = 3))
  ...
  test_that ("same results with formula interface", {
    set.seed(32)
    default_nfindr <- nfindr(.testdata$x[,1:2], p = 3)$indices
    expect_equal(formula_nfindr$indices, default_nfindr)
  })  
  
  test_that ("check conversion of classes", {
    expect_equal(suppressWarnings(nfindr(~ spc, laser, p = 2)$indices), .correct.laser)
  })
  
  test_that ("Formula Interface", {
    expect_equal(formula_nfindr$indices, .correct)
    set.seed(32)
    expect_equal(nfindr (~ ., as.data.frame(.testdata$x[,1:2]), p = 3)$indices, .correct)  
  })
